...
<template>
  <div>
    ...
    >{{`${$Amplify.I18n.get('Mobile Number')}${isPhoneNumberRequired?' *':''}`}}</div>

    <div v-bind:class="onsenClasses.selectInput">
      <select
        class="select-input"
        v-model="countryCode"
      ...
      <input
        ...
        v-bind:class="[onsenClasses.input, isInvalid ? 'invalid': '']"
      ...
      />
      <div v-bind:class="onsenClasses.hint">{{`[+${countryCode}${local_phone_number}]`}}</div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    getPlaceholder() {
      return this.placeholder || "Enter your Mobile Number";
    }
  },
  methods: {
